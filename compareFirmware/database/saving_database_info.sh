...
du -ab --max-depth=3 | sort -h | grep bin | grep rev | grep samd20 >> revision_bin_sizes.txt
echo "" >> revision_bin_sizes.txt
echo "SAMD21 bin sizes [Byte]:" >> revision_bin_sizes.txt
du -ab --max-depth=3 | sort -h | grep bin | grep rev | grep samd21 >> revision_bin_sizes.txt
