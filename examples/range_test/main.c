...
static char test_sender_stack[GNRC_NETIF_BUS_NUMOF][THREAD_STACKSIZE_MAIN];
...
static int _range_test_cmd(int argc, char** argv)
{
    ...
    struct sender_ctx ctx[GNRC_NETIF_BUS_NUMOF];
...
static void* range_test_server(void *arg)
{
    ...
    while (1) {
        ...
        case TEST_PONG:
        {
            ...
            range_test_add_measurement(netif, rssi, pp->rssi, xtimer_now() - pp->ticks);
            break;
        }
        default:
            printf("got '%s'\n", (char*) pkt->data);
